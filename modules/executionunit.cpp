...
ExecutionUnit::ExecutionUnit(QObject *parent,VariableHandler* varHandler, QHash<QString,QVariant> inputHash,QList<SystemContainer*> systems) : QObject(parent)
{
...
ExecutionUnit::ExecutionUnit(QObject *parent,VariableHandler* varHandler, QMap<QString,QVariant> inputHash) : QObject(parent)
{
    envContainer = new EnvironmentContainer(this,varHandler);
    for(QString key : inputHash.keys()){
        QVariant value = inputHash.value(key);
        ...
        if(key.endsWith(".exec")||key=="commandline"){
            m_execType = key.split(".").first();
            m_execCommand = value.toString();
            m_execString = m_execCommand;
        }
    }
    if(!m_execString.isEmpty())
        s_validUnit = true;
